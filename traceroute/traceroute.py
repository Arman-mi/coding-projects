
import util

# Your program should send TTLs in the range [1, TRACEROUTE_MAX_TTL] inclusive.
# Technically IPv4 supports TTLs up to 255, but in practice this is excessive.
# Most traceroute implementations cap at approximately 30.  The unit tests
# assume you don't change this number.
TRACEROUTE_MAX_TTL = 30

# Cisco seems to have standardized on UDP ports [33434, 33464] for traceroute.
# While not a formal standard, it appears that some routers on the internet
# will only respond with time exceeeded ICMP messages to UDP packets send to
# those ports.  Ultimately, you can choose whatever port you like, but that
# range seems to give more interesting results.
TRACEROUTE_PORT_NUMBER = 33434  # Cisco traceroute port number.

# Sometimes packets on the internet get dropped.  PROBE_ATTEMPT_COUNT is the
# maximum number of times your traceroute function should attempt to probe a
# single router before giving up and moving on.
PROBE_ATTEMPT_COUNT = 3

class IPv4:
    # Each member below is a field from the IPv4 packet header.  They are
    # listed below in the order they appear in the packet.  All fields should
    # be stored in host byte order.
    #
    # You should only modify the __init__() method of this class.
    version: int
    header_len: int  # Note length in bytes, not the value in the packet.
    tos: int         # Also called DSCP and ECN bits (i.e. on wikipedia).
    length: int      # Total length of the packet.
    id: int
    flags: int
    frag_offset: int
    ttl: int
    proto: int
    cksum: int
    src: str
    dst: str

    def __init__(self, buffer: bytes):
        self.b = ''.join(format(byte, '08b') for byte in [*buffer])

        self.version = int(self.b[0:4],2)
        self.header_len = int(self.b[4:8],2) * 4 
        self.tos = int(self.b[8:16],2)
        self.length = int(self.b[16:32],2)
        self.id = int(self.b[32:48],2)
        self.flags = int(self.b[48:51],2)
        self.frag_offset = int(self.b[51:64],2)
        self.ttl = int(self.b[64:72],2)
        self.proto = int(self.b[72:80],2)
        self.cksum = int (self.b[80:96],2)
        ip_list = []
        for i in range(96,128,8):
            
            ip_list.append((str(int(self.b[i:i+8], 2))))
        self.src = ".".join(ip_list)

        ip_list_another_one = []
        for j in range(128,160,8):
            ip_list_another_one.append(str(int(self.b[j:j+8],2)))
        self.dst = ".".join(ip_list_another_one)





    def __str__(self) -> str:
        return f"IPv{self.version} (tos 0x{self.tos:x}, ttl {self.ttl}, " + \
            f"id {self.id}, flags 0x{self.flags:x}, " + \
            f"ofsset {self.frag_offset}, " + \
            f"proto {self.proto}, header_len {self.header_len}, " + \
            f"len {self.length}, cksum 0x{self.cksum:x}) " + \
            f"{self.src} > {self.dst}"


class ICMP:
    # Each member below is a field from the ICMP header.  They are listed below
    # in the order they appear in the packet.  All fields should be stored in
    # host byte order.
    #
    # You should only modify the __init__() function of this class.
    type: int
    code: int
    cksum: int

    def __init__(self, buffer: bytes):
        self.b = ''.join(format(byte, '08b') for byte in [*buffer])
        self.type = int(self.b[0:8],2)
        self.code = int(self.b[8:16],2)
        self.cksum = int(self.b[16:32],2)
        
        



    def __str__(self) -> str:
        return f"ICMP (type {self.type}, code {self.code}, " + \
            f"cksum 0x{self.cksum:x})"


class UDP:
    # Each member below is a field from the UDP header.  They are listed below
    # in the order they appear in the packet.  All fields should be stored in
    # host byte order.
    #
    # You should only modify the __init__() function of this class.
    src_port: int
    dst_port: int
    len: int
    cksum: int

    def __init__(self, buffer: bytes):
        self.b = ''.join(format(byte, '08b') for byte in [*buffer])

        self.src_port = int(self.b[0:16],2)
        self.dst_port = int(self.b[16:32],2)
        self.len = int(self.b[32:48],2)
        self.cksum = int(self.b[48:64],2)





    def __str__(self) -> str:
        return f"UDP (src_port {self.src_port}, dst_port {self.dst_port}, " + \
            f"len {self.len}, cksum 0x{self.cksum:x})"

# TODO feel free to add helper functions if you'd like

def traceroute(sendsock: util.Socket, recvsock: util.Socket, ip: str) \
        -> list[list[str]]:
    """ Run traceroute and returns the discovered path.

    Calls util.print_result() on the result of each TTL's probes to show
    progress.

    Arguments:
    sendsock -- This is a UDP socket you will use to send traceroute probes.
    recvsock -- This is the socket on which you will receive ICMP responses.
    ip -- This is the IP address of the end host you will be tracerouting.

    Returns:
    A list of lists representing the routers discovered for each ttl that was
    probed.  The ith list contains all of the routers found with TTL probe of
    i+1.   The routers discovered in the ith list can be in any order.  If no
    routers were found, the ith list can be empty.  If `ip` is discovered, it
    should be included as the final element in the list.
    """
    res =[]
    # # TODO Add your implementation
    for ttl in range(1, TRACEROUTE_MAX_TTL+1):
        TTL_dups = set()
        accepted_ports = set()
        probe = 0
        ports_sofar = set()
        acceptedProbes = 0 
        addresses = []
        nulls = 0


        while True:
            if probe < PROBE_ATTEMPT_COUNT:
                sendsock.set_ttl(ttl)
                msg = "naruto is the goat suiiiiii"
                destPort = 33464 + ttl * 8 + probe
                ports_sofar.add(destPort)
                # changing 33464 to destPort 
                sendsock.sendto(msg.encode(),(ip,destPort))

                probe+=1
            if probe - len(accepted_ports) ==0:
                break

            if not recvsock.recv_select():
                nulls +=1
            else:
                nulls = 0
                buf, address = recvsock.recvfrom()

                headers = helper_header_constructor(buf)
                if headers is None:
                    continue
                IP_header , ICMP_header, UDP_header = headers

                ip_address = IP_header.src

                if UDP_header.dst_port not in ports_sofar:
                    continue
                if UDP_header.dst_port in accepted_ports:
                    continue
                accepted_ports.add(UDP_header.dst_port)

                if ip_address not in addresses:
                    addresses.append(ip_address)
                # acceptedProbes +=1
                if probe >= PROBE_ATTEMPT_COUNT and nulls>=1:
                    break

                # print(res)
                icmp_error_code = 3
                if ICMP_header.type == icmp_error_code and ICMP_header.code == icmp_error_code:
                    res.append(addresses)
                    
                    
                    util.print_result(addresses,ttl)
                    return res
            if acceptedProbes >= PROBE_ATTEMPT_COUNT:
                break
            if probe>= PROBE_ATTEMPT_COUNT and nulls >= 1:
                break
            
        res.append(addresses)
        util.print_result(addresses,ttl)
    return res

        


# def helper_b1_b4(ip_header,icmp_header ):
#     #Meow?
#     if ip_header.proto != 1:
#         return False
    

def helper_header_constructor(buf):
    if len(buf) < 20:
        return None  
    try:                                            
        ip_header = IPv4(buf)
    except (ValueError, IndexError):
        return None
    if ip_header.header_len +8 > len(buf):
        return None
    
    ICMP_index = ip_header.header_len
    ICMP_buffer = buf[ICMP_index:]
    ICMP_header = ICMP(ICMP_buffer)
    if ICMP_header.type !=3 and ICMP_header.type !=11:
        return None 
    if ICMP_header.type == 11 and ICMP_header.code != 0:
        return None
    if ip_header.proto != 1:
        return None
    
    payload = buf[ICMP_index + 8 :]
    try:
        innerIP = IPv4((payload[:20]))
    except(ValueError,IndexError):
        return None
    innerLen = innerIP.header_len
    if len(payload) < innerLen + 8 :
        return None
    UDP_buf = payload[innerLen:innerLen + 8 ]
    try:
        udp_header = UDP(UDP_buf)
    except (ValueError,IndexError):
        return None
    return ip_header, ICMP_header, udp_header

    

    


    





    
    

    
    






                # print(ip_header.src)
                # i +=1
                # if ip_header.proto != 1:
                #     continue
                # if not helper_b1_b4(ip_header,ICMP_header):
                #     continue






        



if __name__ == '__main__':
    args = util.parse_args()
    ip_addr = util.gethostbyname(args.host)
    print(f"traceroute to {args.host} ({ip_addr})")
    traceroute(util.Socket.make_udp(), util.Socket.make_icmp(), ip_addr)